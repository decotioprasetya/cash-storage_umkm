...
const Inventory: React.FC = () => {
  const { state, addBatch, deleteBatch } = useApp();
    ...
    <div className="space-y-6 lg:space-y-8 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 lg:gap-8">
        <div className="xl:col-span-2 space-y-6">
          <div className="bg-white dark:bg-slate-900 rounded-[1.5rem] lg:rounded-[2rem] border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden">
            <div className="p-4 lg:p-6 border-b border-slate-50 dark:border-slate-800 space-y-4">
              <div className="relative">
                ...
                <input 
                  ...
                  className="w-full pl-10 pr-4 py-2.5 bg-slate-50 dark:bg-slate-800 border-2 border-slate-100 dark:border-slate-800 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white dark:focus:bg-slate-700 text-[10px] transition-all uppercase text-black dark:text-white font-black"
                  value={search}
              ...
              <table className="w-full text-left min-w-[500px]">
                <thead>
                  <tr className="bg-slate-50/50 dark:bg-slate-800/50 text-slate-400 text-[9px] uppercase font-black tracking-widest border-b dark:border-slate-800">
                    <th className="px-6 lg:px-8 py-4">Produk</th>
                    <th className="px-6 lg:px-8 py-4">Tipe</th>
                    <th className="px-6 lg:px-8 py-4">Sisa Stok</th>
                ...
                <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
                  {filteredBatches.map((batch) => (
                    <tr key={batch.id} className={`hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors group ${batch.currentQuantity === 0 ? 'opacity-40 grayscale-[0.5]' : ''}`}>
                      <td className="px-6 lg:px-8 py-4">
                        <p className="font-black text-[10px] lg:text-[11px] text-slate-900 dark:text-slate-100 uppercase leading-none">{batch.productName}</p>
                        <p className="text-[8px] text-slate-400 font-bold mt-1 tracking-tighter uppercase opacity-60">ID: {batch.id.slice(0, 8)}</p>
                      </td>
                      <td className="px-6 lg:px-8 py-4">
                  ...
                  {filteredBatches.length === 0 ? (
                    <tr>
                      <td colSpan={6} className="px-6 lg:px-8 py-20 text-center">
                        <div className="flex flex-col items-center justify-center opacity-20">
                          <Boxes size={48} className="text-slate-400 mb-4" />
                          <p className="text-slate-600 dark:text-slate-400 font-black uppercase text-[10px] tracking-widest">
                            {showEmpty ? 'Gudang Benar-benar Kosong' : 'Tidak Ada Stok Aktif'}
                          </p>
                        </div>
                      </td>
                    </tr>
                    ...
                    <tr className="bg-slate-900 dark:bg-slate-950 text-white">
                       <td colSpan={4} className="px-6 lg:px-8 py-4 text-[9px] lg:text-[10px] font-black uppercase tracking-widest text-right">
                         Total Nilai Aset Terpilih:
                       </td>
                       <td colSpan={2} className="px-6 lg:px-8 py-4">
                         <p className="text-xs lg:text-sm font-black tracking-tighter text-blue-400">{formatIDR(totalValueFiltered)}</p>
                       </td>
                    </tr>
                  )}
                </tbody>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-[1.5rem] lg:rounded-[2rem] border border-slate-100 dark:border-slate-800 shadow-sm flex flex-col max-h-[500px] lg:max-h-[700px]">
          <div className="p-4 lg:p-6 border-b border-slate-50 dark:border-slate-800 flex items-center gap-3">
          ...
          <div className="flex-1 overflow-y-auto p-3 lg:p-4 space-y-2 lg:space-y-3 no-scrollbar text-white">
